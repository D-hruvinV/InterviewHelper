...
export default function Interview({ topic, onFinish }) {
  ...
  const generateQuestion = async (lastAnswer = "") => {
    setLoading(true);
    const res = await axios.post(`${import.meta.env.BACKEND_URL}/api/generate-question`, {
      topic,
      history,
      lastAnswer
    });
  ...
  const handleSubmit = async () => {
    setLoading(true);
    const res = await axios.post(`${import.meta.env.BACKEND_URL}/api/analyze-answer`, {
      question,
      answer,
    });
    ...
    if (step + 1 < MAX_QUESTIONS) {
      ...
      setLoading(true);
      const overallRes = await axios.post(`${import.meta.env.BACKEND_URL}/api/overall-grade`, {
        history: newHistory
      });
